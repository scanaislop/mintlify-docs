...
description: "Scan your first project, read your score, repair issues, and add a CI quality gate. Four steps, no upfront configuration."
---
...
<Steps>
  ...
  <Step title="Repair issues">
    ...
    aislop fix --dry-run    # print the plan without writing files
    aislop fix --changes    # only files changed since HEAD
  ...
  <Step title="Add to CI">
    ...
    The fastest way to add aislop to **GitHub Actions**, with no version to maintain:
  ...
  <Step title="Give aislop to your coding agent">
    Install the skill so the agent knows when to scan, then drop repo instructions it will actually load.

    ```bash
    npx skills add scanaislop/skills
    ```

    Paste this into the agent if you want it to do the setup:
    ...
    Copy-paste prompt, AGENTS.md template, skill URL, and MCP snippet: [For agents](/agents).
  </Step>
</Steps>
