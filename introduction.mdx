...
<CardGroup cols={2}>
    <Card title="Formatting" icon="paintbrush">
        ...
        fmt** (Rust), **rubocop** (Ruby), **php-cs-fixer** (PHP), **dotnet format** (C#), and **clang-format** (C/C++).
    </Card>
    <Card title="Linting" icon="triangle-exclamation">
        ...
        **clippy** (Rust), **expo-doctor** (Expo/React Native), **Roslynator / InspectCode** (C#), and **cppcheck / clang-tidy** (C/C++).
    </Card>
...
aislop analyses 10 language targets. Optional system tools add deeper language-specific coverage; built-in checks still run when those tools are absent.
...
<Note>
    If a repository is primarily written in an unsupported language such as Swift or Kotlin, aislop withholds the
