...
Since 0.15.0, deductions are divided by file count before the scoring curve. A large project and a small one with the same habits land on the same number. Copying a tree three times no longer tanks the score for being larger.

- A few issues in a large codebase do not dominate the number
- A small dirty project is not padded by size
- Rule caps scale with project size, so they mean the same thing at any scale
...
Increase `smoothing` (default: 5) to reduce penalty spikes when scanning large codebases with accumulated technical debt.
