...
jobs:
  quality-gate:
    ...
    steps:
      - uses: actions/checkout@v4
      - uses: scanaislop/aislop@v1        # or pin: @v0.11.0
        with:
          version: latest                 # CLI version; pin e.g. "0.11.0" for reproducibility
```
...
| `version` | No | `"latest"` | npm aislop CLI version to run, e.g. `"0.11.0"`. Independent of the action ref in `uses:`. |
